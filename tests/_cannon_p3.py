...
for stp in range(4):
    t=(stp+1)/4;s=st(t)
    for i,d in enumerate(ds):
        if i in [1,3,5]:  # 对话组: 大半径旋转(180-200)
            ang=2*math.pi*i/6+math.pi/6+0.3*math.pi*s
            r=180+20*math.sin(s*math.pi)
            x=280+r*math.cos(ang);y=280+r*math.sin(ang)
            z=160+40*math.sin(s*math.pi+i)
        elif i==6:  # 中心微动
            x=280+25*math.sin(s*2*math.pi)
            y=280+25*math.cos(s*3*math.pi)
            z=170
        else:  # 背景组: 小半径微动(120-140)
            ang=2*math.pi*i/6+math.pi/6
            r=120+20*math.sin(s*math.pi)
            x=280+r*math.cos(ang);y=280+r*math.sin(ang)
            z=130
        d.move2(i3(x),i3(y),i3(z))
        if i in [1,3,5]: d.TurnOnAll("#ffdd59")
        elif i==6: d.TurnOnAll("#ffdd59")
