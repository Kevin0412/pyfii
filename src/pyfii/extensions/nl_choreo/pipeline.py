...
def _build_pyfii_usage_guide_context() -> str:
    ...
    return (
        ...
        "- 禁止用 random/random walk 当作动作设计；随机搜索只能用于离线探索，最终脚本必须是可解释、可复现的手写/结构化动作短句。\n"
        "- 禁止用单个全局 rotation 参数驱动全片；可以短暂旋转，但整段必须包含反向、切入、错层、分组换位和非环形结构。\n"
...
def _build_direct_codegen_prompt(
    ...
    prompt = (
        ...
        "- 不要用 random/random walk 直接生成动作；如果需要搜索，只能作为离线候选探索，最终输出必须是确定性 phrase/action-score。\n"
        "- 不要用单个全局 rotation 或 ribbon 场统治全片；全片必须有反向切割、错层穿插、压缩爆开、分组换位等明确动作意图。\n"
        ...
        "22) 禁止 random/random walk 作为最终动作生成方式；最终动作必须由命名 phrase、目标点、分组关系或参数轨迹确定。\n"
        "23) 禁止全片单一全局逆/顺时针旋转；允许局部旋转，但必须穿插反向切割、错层、非均匀时间和分组角色交换。\n"
        "\n"
