...
3. pyfii模拟
    ...
    1. 读入文件并计算轨迹
        ...
        原理是读入```.fii```工程后定位各动作组的```webCodeAll.xml```，再把XML块程序解析成时间轴事件列表```dots```，最后将事件离散化为轨迹序列。当前实现中，这一步并不是通用XML DOM解析，而是基于字符串和缩进层级扫描识别```Goertek_*```/```block_*```节点，并递归展开```controls_repeat```。
        ...
        对于旋转与移动的耦合，当前实现是解耦处理的：平移由```dots2line```生成，朝向由```dots2angle```按角速度单独积分，灯光由```dots2led```独立采样，最后在统一时间轴上合并。因此当前仿真没有建模角加速度，也没有让姿态反作用于位移。
