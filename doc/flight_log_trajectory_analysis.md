...
1. **理想条件与非理想条件必须分开**：pyfii 的理想条件是上一段 `move2` 已按梯形曲线到点并减速到 `v=0`，下一段才开始。`171502-98101` 的 pyfii 重放有 **9 次完整动作交接、0 次打断**；真机对应动作的段内 3D 到点残差中位数为 **7.1 cm**、90 分位为 **14.5 cm**，现有理想模型可继续作为默认基线。
...
5. **warning 必须按独立动作事件统计**：网格日志中的 **244 条 warning 是 4 个打断事件在 200 Hz 循环中逐帧重复记录**；复合航线的 **1310 条 warning 帧对应 25 个独立打断**。网格真机在可判定的转向处明显减速，当前打断模型将转向残差的平均误差由理想到点基线的 **42.0 cm** 降到 **23.0 cm**，说明“先减速再换段”的处理方向合理，但参数仍偏理想。
...
7. **复飞一致性高**：`162913`、`163106`、`180602` 三次复合航线清洗后中位 3D 差约 **4.9-8.0 cm**，90 分位约 **17.4-32.3 cm**。真实飞行具备可重复性，说明按完成/打断条件比较 pyfii 指令时序基线具有意义；但它还不足以证明应替换梯形模型。
...
4. 默认位姿、坐标越界、`Dead Battery`、`FLIP` 后定位发散等样本只用于定位质量判断，不参与运动残差。

真实实验使用 fwfii 在线发包，而日志没有逐条命令的发送时间戳；pyfii API 重放与在线编码层也不完全相同。因此，本文不使用一个全局 offset 强行拟合全部通道：水平轨迹按首次持续 XY 运动对齐，高度按首次离地对齐，动作是否完成则主要使用段内目标残差和转向减速证据。
...
真机动作窗口由脚本中的 `Delay` 累积得到，并给在线指令响应保留 0.8 s 余量。9 个目标的段内 3D 残差中位数为 **7.1 cm**，90 分位为 **14.5 cm**，最大为 **17.1 cm**；各目标附近的最小可观测 XY 速度中位数为 **10.0 cm/s**。因此在 pyfii 判定满足完成条件的样本中，真机也表现为接近目标并明显减速，现有“直线 + 梯形速度 + 到点停止”适合作为理想基线；5 Hz 遥测仍不足以验证连续时间内是否精确到 `v=0`。

![理想完成与提前打断分开统计](images/fig11_ideal_vs_interrupted.png)

图 11 左侧只统计理想完成样本，右侧只统计提前打断样本。两类数据不共同拟合一个速度模型。
...
图 1 中蓝线是用于对照的理想完成轨迹，黑色虚线是 pyfii 实际的“按时打断、沿旧方向制动到零、从停点换段”轨迹，红线是真实清洗后轨迹。真实残差按每个动作自己的转向窗口计算，不从整段日志全局搜索航点，以免把降落后的错误坐标当成到点样本。

| 被打断动作 | 目标 XY | pyfii 制动停点残差 | 真机转向点残差 | 转向窗口最小可观测 XY 速度 |
|:--|:--|--:|--:|--:|
| 航点 1 | (320, 40) | 29.1 cm | 39.1 cm | 22.2 cm/s |
| 航点 2 | (40, 120) | 7.9 cm | 46.1 cm | 25.5 cm/s |
| 航点 3 | (320, 200) | 27.9 cm | 43.7 cm | 不判定：AprilTag 停帧/跳变 |
| 航点 4 后降落 | (40, 280) | 11.4 cm | 39.2 cm | 5.0 cm/s |

四个真机转向点的平均 XY 残差为 **42.0 cm**。若使用理想到点基线，残差预测恒为 0，平均误差就是 **42.0 cm**；使用 pyfii 当前打断制动分支后，平均误差降为 **23.0 cm**，停点与真机转向点的平均平面距离为 **23.4 cm**。因此实验支持保留“动作未完成时先减速再处理下一动作”的机制，当前模型只是比真机更容易接近航点。
...
这个残差显著大于真实复飞之间的差异，说明真实飞行相对 pyfii 指令时序基线存在稳定偏差；但残差里也包含 pyfii 表达范围（如 `z=60`）和脚本语义差异，不能直接推出某个复杂替代模型。
...
|:--|:--|:--|:--|
| 理想完成 | 到点并减速到零后换段 | `171502-98101` 段内 3D 残差中位 7.1 cm、P90 14.5 cm | 理想梯形模型保留 |
| 提前打断 | 沿旧方向以 400 cm/s² 制动，再从停点换段 | 网格转向点残差均值 42.0 cm，可用窗口均明显减速 | 打断减速机制保留 |
| 打断精度 | pyfii 停点残差均值 19.1 cm | 真机停点更早，模型与真机残差 MAE 23.0 cm | 当前分支偏理想，可校准但不能直接替换 |
...
1. **理想动作模型不改**。上一段有足够时间完成时，继续采用现有“直线 + 梯形/三角速度 + 到点归零”。`171502-98101` 的 9 个完整交接和厘米级段内残差支持这一选择。
2. **动作未完成分支保留“先减速、后换段”**。网格实验显示真机也会在目标前减速转向；当前分支比理想到点基线更接近真实停点，没有证据支持改成不断速航点融合，更没有必要引入 jerk 模型。
3. **warning 改为每个打断事件只报一次**。事件至少应包含：旧动作目标、下一指令时刻、打断时位置残差、打断时速度、预计制动时间、制动停点以及是否有中间指令被跳过。逐帧 warning 数可以作为内部调试量，但不能再对外表示“动作数”。
4. **模拟输出明确标记 `completed` 与 `interrupted`**。碰撞检查和轨迹图应使用实际指令时序分支；理想完成轨迹可以同时展示，但不能替代非理想分支参与残差计算。
...
最终结论是：**pyfii 可以优化，但优化对象是非理想动作的事件识别、可观测输出和后续可校准参数；现有理想梯形建模应保持不变，当前“动作未完成后先减速”的处理也应保留。**
