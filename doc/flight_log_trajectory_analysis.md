...
3. **本文采用 pyfii 梯形速度曲线作为统一分析基线**：所有清洗后可用的飞行动作，都先用 pyfii 现有“直线 + 梯形速度曲线”重放，得到指令基线；再量化真实轨迹相对基线的切角、欠冲、时序压缩、垂直速度偏差和安全裕度。
...
图 3 使用 pyfii 指令时序基线，而不是“飞完再等”的几何基线。如果把完整飞行时间与 `Delay/WaitStable` 串行相加，理想曲线会被整体拉长，速度残差会被误读。按指令墙钟时序重放后，pyfii 仍是平顶梯形，真实速度仍然连续、峰值圆钝、航点附近不归零；这才是需要解释的物理差异。
...
图 2 同样使用指令时序基线。真实 `Delay/WaitStable` 是发出命令后的墙钟等待，不是“等 move 完成后再计时”；因此不能把每段完整飞行时长和 Delay 简单相加。按指令时序对齐起飞后，pyfii 基线约 **18.26 s** 落地，真实日志约 **18.04 s** 到近地，二者相差约 **0.22 s**，不足以支持“提前降落”的结论。修正对时后，剩下的差异主要来自近地 AprilTag 定位跳变和真实降落剖面。
...
3. **当前数据支持的确定性修正是有限的**：按真实指令时序对齐 pyfii 基线、区分 full-completion 几何基线和 command-timed 基线、清洗 AprilTag 定位异常、以及检查 `VelZ/AccZ` 是否需要进入模拟通道。
