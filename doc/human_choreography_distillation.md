...
| 校园作品A | `周深 - 向光而行 （赵-孟）2.mp3`，约 107.7 BPM，onset 密度约 2.71/s | 7/12 个动作边界在 1.5s 内，7/12 个在 3s 内 | 0-36s 与音乐边界贴合较强；44s 后检测到的频谱 novelty 较少，但动作仍按歌词/视觉句子继续分段，说明不能只依赖算法边界 |
| competition_test_62 | 当前目录和渲染视频未找到可解码音频 | 不能判断 | 只能做动作压力样本；如果要做音乐蒸馏，需要补原始音乐或带音轨视频 |
| output/1（校园作品B-Trip） | `TRIP_01.mp3`，约 123.0 BPM，onset 密度约 5.61/s | 6/7 个动作边界在 1.5s 内，7/7 个在 3s 内 | 音乐前 32s 低中能段对应全体同步的 waypoint 遍历；32s 能量跃升后编舞进入分组交替，60s 能量骤降时编舞回场降落；6s 动作边界略早于音乐进入（前奏），但整体 6s/45s/46s/50s/60s/65s 切段与音乐结构贴合 |
...
| 校园作品A | 约 69.5s，未完成约 248，最小平面距约 7cm | 未完成约 8873，最小平面距接近 0 | 高运动量和多层次可学，执行层必须重做 |
| output/1（校园作品B-Trip） | 约 65.9s，未完成 0，最小平面距约 74cm | 约 66.4s，未完成 0，最小平面距约 74cm | waypoint 几何设计和分组交替可学，缺视频和 pyfiiCode.py 限制深度蒸馏 |
...
- pyfiiCode.py：有，可直接读取设计意图。生成脚本为 `tests/dntg20220730_v3.py`——这是理解大闹天宫设计意图的最直接入口，展示了如何用复数旋转、利萨如轨迹、角色映射等数学方法生成编队。
...
- 音乐：`output/无人区/动作组/无人区.mp3`，约 101 BPM，onset 密度 4.82/s；音乐全长 70.1s，编舞覆盖约 66s。能量结构极具戏剧性：0-16s 低能留白（0.15-0.31），20-24s 突然爆发（0.79 峰值），24-36s 高能持续（0.57-0.66），36-68s 中高能维持（0.39-0.59）。
- 视频：无（GPT-5.5 文档中列出的视频文件在当前目录未找到）
- pyfiiCode.py：无（Fii 原软件 Blockly 导出）
- 设备：F600（第二个 F600 样本，与太空电梯共享机型）
- 结构：段落细碎，约在 0、1、3、5、6、10、16、17、18、19、20、30、31、38、39、40、41、49、51、52、54、55、64、65s 附近变化。前段（0-16s）短切密集配合低能留白，20-30s 高能爆发段大中心移动，后段持续中高能维持。
- 几何设计：XY 范围 X[40,520]，无预定义 waypoint。20-30s 高能段中心移动接近全场推进，呼应音乐爆发。
- 灯光设计：中等密度，段间切换配合颜色变化。
- 空间叙事（核心亮点）：低能段的短切和留白不是"空白"而是"负空间"——制造静谧/紧张对峙感。20s 能量突然爆发时编舞响应大中心移动和全场推进。"突然加速"和"负空间"是主题表达的核心手法，与大闹天宫的持续高密度、开启新征程的渐进推进形成对比。
- 历史视觉价值：适合学习"留白→爆发"的对比叙事、负空间的戏剧性运用、能量突变时的编舞响应（大中心移动而非均匀加速）。
- 现代验证风险：ignore_acc=True 下 68 次未完成（较少），但 ignore_acc=False 下 6106 次——说明加速度模型对此作品冲击极大。换位需要拆段、增加中间点和错峰。
- 可蒸馏原则：负空间/留白的戏剧性运用、能量突变→大中心移动的响应模式、F600 机型对比参考。
...
```json
{
  ...
  "segments": [
    {
      "time_range": [0, 16],
      "intent": "低能留白段：短切密集但动作幅度小，制造静谧/对峙的负空间",
      "music_cue": "低能（0.15-0.31），暗色、稀疏",
      "beat_policy": "短切（1-3s 级），动作克制",
      "formation_notes_2d": "小范围 XY 移动，保持张力",
      "spatial_notes_3d": "Z 低空维持",
      "motion_primitives": ["负空间", "留白", "对峙"],
      "light_notes": "暗色灯光配合"
    },
    {
      "time_range": [16, 20],
      "intent": "过渡：能量开始爬升，动作幅度扩大",
      "music_cue": "能量爬升（0.15→0.35）",
      "formation_notes_2d": "XY 范围开始扩大",
      "spatial_notes_3d": "Z 开始上升",
      "motion_primitives": ["过渡", "蓄力"],
      "light_notes": "灯光配合能量变化"
    },
    {
      "time_range": [20, 30],
      "intent": "高能爆发：音乐能量峰值（0.79），编舞响应大中心移动+全场推进",
      "music_cue": "能量爆发峰值（0.79→0.64→0.57），最强脉冲段",
      "beat_policy": "大动作跟能量峰值，全场推进",
      "formation_notes_2d": "XY 大范围移动，中心偏移接近全场",
      "spatial_notes_3d": "Z 全范围跃升",
      "motion_primitives": ["爆发", "全场推进", "大中心移动"],
      "light_notes": "灯光密度上升配合峰值",
      "execution_risk": ["换位未完成（现代模式）"],
      "repair_strategy": ["增加中间点", "错峰启动"]
    },
    {
      "time_range": [30, 40],
      "intent": "高能维持：持续中高能，细碎段落保持张力",
      "music_cue": "高能持续（0.57-0.66），30-36s onset 密集",
      "beat_policy": "短切段呼应 onset 密度",
      "formation_notes_2d": "XY 全场维持",
      "spatial_notes_3d": "Z 高位维持",
      "motion_primitives": ["高能维持", "细碎段落"],
      "light_notes": "灯光密集配合"
    },
    {
      "time_range": [40, 55],
      "intent": "中高能持续：能量略降但仍保持，编舞维持场域",
      "music_cue": "中高能（0.39-0.54），稳定但不如前段激烈",
      "formation_notes_2d": "XY 全场覆盖",
      "spatial_notes_3d": "Z 分化",
      "motion_primitives": ["中高能维持"],
      "light_notes": "灯光持续"
    },
    {
      "time_range": [55, 66],
      "intent": "收束：能量渐退，编舞收束",
      "music_cue": "能量中高→衰退（0.55→0.12）",
      "formation_notes_2d": "队形收束",
      "spatial_notes_3d": "Z 下移→land",
...
- 来源：`output/校园作品A/校园作品A.fii`
- 音乐：`output/校园作品A/动作组/周深 - 向光而行 （赵-孟）2.mp3`，约 108 BPM，onset 密度 2.81/s；音乐全长 71.9s，编舞覆盖约 70s（几乎全曲）。能量结构渐进上升：0-24s 低能（0.20-0.40），24s 起渐进爬升，36-72s 持续高能（0.60-0.70）。能量曲线几乎单调上升——呼应"向光而行/凌云志"的上升主题。
- 视频：无（GPT-5.5 文档中列出的视频文件在当前目录未找到）
- pyfiiCode.py：无（Fii 原软件 Blockly 导出）
- 设备：F600（第三个 F600 样本）
- 结构：约在 0、4、9、11、20、23、35、36、44、53、57、63、69s 形成段落。前半（0-36s）与音乐边界贴合较强，后半更多跟随歌词/视觉句子而非算法边界。
- 几何设计：XY 范围 X[100,460]——相对窄，不如其他作品极端。高度层和运动密度是主要表达手段。
- 灯光设计：中等密度。
- 渐进上升叙事（核心亮点）：音乐能量几乎单调上升（0.20→0.70），编舞围绕"上升/凌云"主题——不是突然爆发而是持续爬升。动作密度和平均路径都很高（ignore_acc=True 下就有 248 次未完成），展现了"高运动量也有段落目的"的可能性。适合学习校园/团队主题里的上升、分组、重组和高运动量段落。
- 3D 观察重点：无视频，但 Z 跨度大，高度层是"凌云"主题的核心视觉表达。应确认高运动量是否仍有清晰可读的空间图案。
- 现代验证风险：ignore_acc=False 下 8873 次未完成（七个样本中最高的之一），最小距离 3.6cm。高运动量和近距离风险需整体重构。未来 agent 不应照搬动作密度，而应提炼"渐进上升、多层次、强重组"的意图后重新求解速度和时间。
- 可蒸馏原则：渐进上升叙事（能量曲线→编舞响应）、高运动量段落组织、F600 机型参考。
  ...
  "segments": [
    {
      "time_range": [0, 4],
      "intent": "起飞，低能起步",
      "music_cue": "低能（~0.21），前奏",
      "motion_primitives": ["takeoff"],
      "light_notes": "无"
    },
    {
      "time_range": [4, 24],
      "intent": "渐进上升：4-24s 多个短段（4、9、11、20、23s），队形逐步展开，高度渐进爬升",
      "music_cue": "低中能渐进（0.21→0.40），能量稳定爬升",
      "beat_policy": "段落渐进，每段小幅推进",
      "formation_notes_2d": "XY 100-460，逐步覆盖",
      "spatial_notes_3d": "Z 渐进上升——"凌云"主题开始",
      "motion_primitives": ["渐进上升", "逐步展开"],
      "light_notes": "段间灯光标记"
    },
    {
      "time_range": [24, 36],
      "intent": "加速上升：35-36s 密集短段，高度和能量同时跃升",
      "music_cue": "能量中高（0.38→0.49→0.63），加速爬升",
      "beat_policy": "35-36s 密集切分，呼应加速",
      "formation_notes_2d": "XY 覆盖扩大",
      "spatial_notes_3d": "Z 加速上升",
      "motion_primitives": ["加速上升", "密集切换"],
      "light_notes": "灯光密度上升"
    },
    {
      "time_range": [36, 53],
      "intent": "高能群舞：44-53s 多个分段，分组、重组、高运动量",
      "music_cue": "持续高能（0.63-0.69），音乐高潮",
      "beat_policy": "多分段高密度，分组交替",
      "formation_notes_2d": "XY 全场覆盖",
      "spatial_notes_3d": "Z 高位维持，多层次",
      "motion_primitives": ["高能群舞", "分组", "重组"],
      "light_notes": "灯光配合分组"
    },
    {
      "time_range": [53, 69],
      "intent": "顶峰与收束：57、63、69s 分段，抵达"凌云"顶峰后收束",
      "music_cue": "能量峰值（0.67-0.70），后微退至 0.62",
      "beat_policy": "57s→63s→69s 三步收束",
      "formation_notes_2d": "队形顶峰定形→收束",
      "spatial_notes_3d": "Z 顶峰→逐步下降→land",
      "motion_primitives": ["顶峰", "收束", "land"],
      "light_notes": "灯光收束"
    }
  ]
}
...
### output/d/2021比赛（早期设计，pyfii 诞生前）

- 来源：`output/d/2021比赛/2021比赛.fii`
- 音乐：`output/d/2021比赛/动作组/Positive Outlook 剪辑背景.mp3`，72.5s，能量结构多峰（12-16s、24-28s、36-40s、44-48s 均有峰值），编舞仅用前 55s。
- 视频：无
- pyfiiCode.py：无（pyfii 诞生前设计，有 `flightled.py` / `testsocket.py` 辅助脚本）
- 特殊说明：无 `inittime` 标签，无灯光设计（LED=0），全部动作靠 `Controls time` 同步。这是用户早期作品，代表了"纯动作编排、无灯光"阶段的设计思路。
- 结构：无法从 inittime 提取段落，需从 Controls time 反推。55s 内完成全部动作，音乐后 17s 空置。
- 动作密度：机1 有 67 个 move2（密度最高，可能是视觉焦点），机4=40、机7=47，其余 20-27。各机任务不均衡，机1/4/7 承担了大部分动作。
- 历史视觉价值：代表了"无灯光、高密度纯动作"的早期设计阶段。与后来大闹天宫（2100 个灯光指令）形成鲜明对比——展示了灯光从 0 到"100ms 级颜色编舞"的演化路径。
- 现代验证风险：ignore_acc=True 下已有 352 次未完成，ignore_acc=False 下 1396 次。最小距离 5.9-10.5cm，风险高。早期设计未考虑加速度模型。
- 可蒸馏原则：纯动作编排的密度极限参考、灯光从无到有的演化参照。
...
## 可进入知识库的经验### output/d/2021比赛（早期设计，pyfii 诞生前）
