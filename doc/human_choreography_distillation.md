...
- 速度设计：每段有独立的 VelXY/VelZ 设置；速度 50-200 cm/s，XML 中出现的 400 是 `VelXY(v, 2v)` 写入的加速度而非速度（2026-06-10 复核 v3 生成脚本订正）。首段高速展开，后续根据几何复杂度切换速度，速度本身参与节奏设计。
...
### output/d/比赛现场程序（修改版）（早期设计，2021比赛 补灯光版）
