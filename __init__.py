...
def read_fii(name):
    ...
    for drone in drones:
        ...
        with open(name+'\\动作组\\'+drone+'\\webCodeAll.xml', "r",encoding='utf-8') as F:
            file = F.read()
        dots.append(dots2line(file,fii=[x,y]))
        t0=max(t0,len(dots2line(file,fii=[x,y])))
    return dots,t0
...
class Fii:
    def __init__(self,name,drones):
        ...
        self.t0=0
        for d in self.ds:
            self.dots.append(dots2line(d.outputString,fii=[d.X,d.Y]))
            self.t0=max(self.t0,len(dots2line(d.outputString,fii=[d.X,d.Y])))
