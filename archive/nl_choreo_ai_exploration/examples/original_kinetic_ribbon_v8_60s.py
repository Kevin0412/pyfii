...
OUTPUT_DIR = REPO_ROOT / "output" / "original_kinetic_ribbon_v8_60s"
PROJECT_NAME = "original_kinetic_ribbon_v8_60s"
...
def field_parameters(time_sec):
    ...
    center_x = 280 + 70 * math.sin(2 * math.pi * 1.30 * u + 0.3)
    center_x += 30 * math.sin(2 * math.pi * 2.70 * u + 1.1)
    center_y = 282 + 65 * math.cos(2 * math.pi * 1.10 * u + 0.55)
    center_y -= 32 * math.sin(2 * math.pi * 2.25 * u - 0.25)
    radius = 196 + 38 * math.sin(2 * math.pi * 1.85 * u - 0.25)
    radius += 26 * math.sin(2 * math.pi * 3.60 * u + 0.75)
    y_scale = 0.78 + 0.18 * (0.5 + 0.5 * math.sin(2 * math.pi * 2.0 * u + 1.05))
    rotation = 0.25 + 2 * math.pi * 3.20 * u
    rotation += 0.62 * math.sin(2 * math.pi * 2.8 * u - 0.15)
    span = 5.15 + 0.55 * (0.5 + 0.5 * math.sin(2 * math.pi * 1.55 * u + 0.7))
    phase = 2 * math.pi * 3.0 * u
...
def ribbon_points(time_sec):
    ...
    for idx in range(DRONE_COUNT):
        ...
        shear = 0.10 * math.sin(2 * math.pi * u * 2.0 + 0.4)
        x = center_x + x0 + shear * y0
        y = center_y + y0
        z = 156 + 38 * math.sin(theta * 1.08 + phase * 0.56)
        z += 14 * math.cos(idx * 1.64 + phase)
...
def main():
    ...
    print("Original kinetic ribbon v8 generated")
