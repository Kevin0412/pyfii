...
OUTPUT_DIR = REPO_ROOT / "output" / "gpt55_template_motion_v2_60s"
PROJECT_NAME = "gpt55_template_motion_v2_60s"
...
def emit_program(music_path: str, render_fps: int, timing_fps: int) -> str:
    # Template v2 保留 GPT-5.5 seed 的角色重排关键帧，但把段内动作改为显式模板库。
...
SCENE_TEMPLATES = {{
    "wide_diagonal_reveal": "curtain_sweep",
    "diamond_expansion": "bloom_pulse",
    "offset_crown_bridge": "hinge_flip",
    "low_double_triangle": "low_scoop",
    "vertical_break_line": "vertical_shutter",
    "ribbon_recoil": "braid_recoil",
    "chevron_lock": "snap_fold",
    "outer_orbit_bridge": "orbit_sling",
    "reverse_half_arc": "half_moon",
    "tilted_diamond": "tilt_bloom",
    "counter_ribbon": "counter_braid",
    "broad_slash": "slash_shear",
    "rotated_triangle_burst": "triangle_twist",
    "compressed_orbit": "compression_ring",
    "falling_chevron": "falling_gate",
    "late_arc_sweep": "late_sweep",
    "final_recoil_ribbon": "final_braid",
    "last_diamond_flare": "flare_lock",
}}
...
def template_strength(template_name):
    strengths = {{
        "curtain_sweep": 26,
        "bloom_pulse": 34,
        "hinge_flip": 30,
    ...
    return strengths.get(template_name, 26)
...
def scene_point_float(scene_name, drone_idx, t):
    ...
    template_name = SCENE_TEMPLATES.get(scene_name, "curtain_sweep")
    phase = 2 * math.pi * drone_idx / DRONE_COUNT + scene_idx * 0.67
    bump = math.sin(math.pi * t)
    strength = template_strength(template_name) * TEMPLATE_GAIN
    rx = base[0] - 280
    ry = base[1] - 280
    ...
    if template_name == "curtain_sweep":
        arc = strength * bump * (0.7 + 0.3 * math.sin(phase))
        ripple = strength * 0.30 * bump * math.sin(2 * math.pi * t + phase)
        vertical = strength * 0.35 * bump * math.cos(math.pi * t + phase)
    elif template_name == "bloom_pulse":
        radial = strength * 0.90 * bump * (0.75 + 0.25 * math.sin(phase))
        arc = strength * 0.25 * bump * math.sin(2 * math.pi * t + phase)
        vertical = strength * 0.45 * bump * math.cos(2 * math.pi * t + phase)
    elif template_name == "hinge_flip":
        arc = strength * 0.75 * bump * alt
        ripple = strength * 0.45 * bump * math.sin(2 * math.pi * t + phase)
        vertical = strength * 0.55 * bump * math.sin(math.pi * t + phase)
    elif template_name == "low_scoop":
        arc = strength * 0.50 * bump * math.sin(phase)
        ripple = -strength * 0.35 * bump * math.cos(math.pi * t)
        vertical = -strength * 0.55 * bump + strength * 0.25 * bump * math.sin(phase)
    elif template_name == "vertical_shutter":
        arc = strength * 0.20 * bump * alt
        ripple = strength * 0.25 * bump * math.sin(2 * math.pi * t + phase)
        vertical = strength * 0.85 * bump * alt
    elif template_name == "braid_recoil":
        arc = strength * 0.85 * bump * math.sin(2 * math.pi * t + phase)
        ripple = strength * 0.45 * bump * math.cos(3 * math.pi * t + phase)
        vertical = strength * 0.35 * bump * math.cos(2 * math.pi * t + phase)
    elif template_name == "snap_fold":
        arc = strength * 0.65 * bump * alt
        ripple = -strength * 0.45 * bump * math.sin(math.pi * t)
        vertical = strength * 0.45 * bump * alt
    elif template_name == "orbit_sling":
        arc = strength * 0.90 * bump * math.cos(2 * math.pi * t + phase)
        ripple = strength * 0.65 * bump * math.sin(2 * math.pi * t + phase)
        vertical = strength * 0.40 * bump * math.sin(math.pi * t + phase)
    elif template_name == "half_moon":
        arc = strength * bump * (0.80 + 0.20 * math.sin(phase))
        radial = -strength * 0.35 * bump * math.cos(math.pi * t + phase)
        vertical = strength * 0.35 * bump * math.sin(2 * math.pi * t + phase)
    elif template_name == "tilt_bloom":
        radial = strength * 0.70 * bump * math.sin(math.pi * t + phase)
        arc = strength * 0.45 * bump
        vertical = strength * 0.45 * bump * math.sin(phase)
    elif template_name == "counter_braid":
        arc = strength * 0.80 * bump * math.sin(2 * math.pi * (1 - t) + phase)
        ripple = -strength * 0.45 * bump * math.cos(2 * math.pi * t + phase)
        vertical = strength * 0.35 * bump * math.sin(phase)
    elif template_name == "slash_shear":
        arc = strength * 0.35 * bump * math.sin(phase)
        ripple = strength * 0.85 * bump * rank
        vertical = strength * 0.30 * bump * math.cos(2 * math.pi * t + phase)
    elif template_name == "triangle_twist":
        arc = strength * 0.75 * bump * math.cos(2 * math.pi * t + phase)
        radial = strength * 0.55 * bump * math.sin(2 * math.pi * t + phase)
        vertical = strength * 0.50 * bump * alt
    elif template_name == "compression_ring":
        radial = -strength * 0.80 * bump * (0.75 + 0.25 * math.cos(2 * math.pi * t + phase))
        arc = strength * 0.45 * bump * math.sin(2 * math.pi * t + phase)
        vertical = strength * 0.40 * bump * math.cos(phase)
    elif template_name == "falling_gate":
        arc = strength * 0.40 * bump * alt
        ripple = strength * 0.25 * bump * math.sin(phase)
        vertical = -strength * 0.80 * bump * (0.70 + 0.15 * (drone_idx % 3))
    elif template_name == "late_sweep":
        arc = strength * bump * (0.85 + 0.15 * math.sin(phase))
        ripple = strength * 0.50 * bump * math.sin(math.pi * t + phase)
        vertical = strength * 0.35 * bump * math.cos(2 * math.pi * t + phase)
    elif template_name == "final_braid":
        arc = strength * 0.75 * bump * math.sin(2 * math.pi * t + phase)
        ripple = strength * 0.35 * bump * math.cos(math.pi * t + phase)
        vertical = strength * 0.30 * bump * math.sin(2 * math.pi * t + phase)
    elif template_name == "flare_lock":
        radial = strength * 0.60 * bump
        arc = strength * 0.35 * bump * math.sin(phase)
        vertical = strength * 0.30 * bump * math.cos(phase)
    return (
        base[0] + nx * arc + ux * ripple + rx * radial,
        base[1] + ny * arc + uy * ripple + ry * radial,
        base[2] + vertical,
    )
...
print("GPT-5.5 template-motion v2 generated")
...
def main() -> None:
    ...
    print("GPT-5.5 template-motion v2 script ready")
