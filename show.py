...
def show(data,t0,music,show=True,save="",FPS=200,ThreeD=False,imshow=[120,-15]):
    t0=int(t0+0.5)+300
    ...
    while k < t0:
        ...
        if ThreeD:
            ...
            for m in range(0,len(aixs),12):
                for n in range(m+12,len(aixs),12):#计算距离
                    ...
                    if distance<50:
                        ...
                        #print(t,distance,int(distance/20),m+1,n+1)#距离太近就输出错误
                        errors.append([(aixs[m][0][0]-21/4*2**0.5,aixs[m][0][1]-21/4*2**0.5,aixs[m][0][2]),(0,0,255),10,1,'sphere'])#错误红圈标记
                        errors.append([(aixs[n][0][0]-21/4*2**0.5,aixs[n][0][1]-21/4*2**0.5,aixs[n][0][2]),(0,0,255),10,1,'sphere'])
            f+=1
            if f==1:
                time_fps=time.time()
                try:
            ...
            img=IIID.show(aixs+lines+errors+texts,center,1280,720,[imshow[0],imshow[1],1,0,0])
            if len(save)==0:
                cv2.imshow('img',img)
                key = cv2.waitKey(1) & 0xff
                #Esc键退出
