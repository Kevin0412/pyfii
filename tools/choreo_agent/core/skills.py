...
PRIMITIVE_SKILLS = [
    ...
    },
    {
        ...
        "when_to_use": "任何错峰大动作、密集承接、对穿/大交叉。mode='wave' 一般错峰；mode='relay' 对穿（顺序接力，零跨组交叉）。",
    ...
    {
        "name": "call-response-safe",
        "function": "call_response_safe",
        "category": "motion",
        "role_fit": ["buildup", "development", "climax"],
        "purpose": "安全分组问答：LLM 只写目标几何，本地按真实接力时序做 safe_assign，再执行双色 group_relay。",
        "when_to_use": "左右/前后两组一问一答、S02/S03 这类需要明显分组错峰但又经常被碰撞门打回的段。",
        "when_not": "只是无交叉的小幅就近承接（best_assign + per-drone loop 更轻）；纯链式旋律线用 chain_follow_safe。",
        "key_params": "prev=当前队形；geo=custom_points 点表或坐标列表；flying_ms 单组飞行时长；gap_ms 两组问答间隔；relay_split='left_right'|'front_back'；colors=(组0色,组1色)。总时长≈2*flying_ms+gap_ms。",
        "safety": "内部等价于 safe_move(mode='relay')：先按 lead 组/response 组真实 delays 调 safe_assign，再用同一组 targets 和时序执行，避免裸 group_relay + best_assign 的“算着安全、实跑相撞”。几何实际要铺开（两带/弧形点表相邻约70-100cm），但不要手动提高 min_xy_cm；默认门槛已足够，真正碰撞由 safe_assign/validator 逐帧验证。",
        "validation_risks": "若同一 XY 巷对穿、目标点过密或首选 gap 清不开，会抛错并要求铺开几何；想精确贴窗时让首选 gap 能过，例如 S02 8.7s 用 flying_ms=4000, gap_ms=700。",
        "example": "geo = custom_points([...左右/前后两带稀疏问答点...], n=len(drones))\nprev = call_response_safe(drones, prev, geo, 4000, gap_ms=700, relay_split='left_right', colors=('#ff6040','#4060ff'), gradient_to=('#ffd166','#66e0ff'))",
        "combines_with": ["safe-move", "handwritten-geometry", "split-groups"],
        "music_fit": "对答式乐句、交替强拍、卡农中段的分组呼应。",
    },
    {
        "name": "chain-follow",
    ...
    {
        "name": "chain-follow-safe",
        "function": "chain_follow_safe",
        "category": "motion",
        "role_fit": ["development", "transition", "expand"],
    ...
    },
    {
        ...
        "when_to_use": "你已经手动用 safe_assign 按完全相同的接力 delays 验过 targets；或作为 call_response_safe/safe_move 的内部执行器。",
        "when_not": "普通 LLM 生成问答段时不要直接用；尤其不要写 group_relay(best_assign(...)) / group_relay(far_assign(...))，那只验证同步路径，不验证真实接力时序。",
        "key_params": "targets 必须已经是按真实接力时序安全分配后的排列；group_ids 用 split_groups(prev, mode=...) 算；flying_ms 单程时长；gap_ms 问答间隔；colors=(组0色,组1色)。",
        ...
        "validation_risks": "裸 group_relay + best_assign/far_assign/mirror_assign 是 S02 碰撞反复返工的高频根因；几何/分配没按接力时序验证会撞。",
        ...
        "combines_with": ["call-response-safe", "split-groups", "safe-assign"],
    ...
    },
    {
        ...
        "key_params": "mirror_assign(prev, geo)。**必须足量错峰**：错峰要大到一架先离开交叉点、另一架才到（小错峰如 i*150 救不了真交叉）；拿不准用 safe_assign(prev, geo, delays=delays, flying_ms=...) 按真实分时轨迹验真挑排列。",
        "safety": "足量顺序错峰才是对穿的安全机制——不要为消碰撞丢掉对穿，但小错峰照撞；分时门与 validator 都逐帧核验，mirror 不是免检。",
        "validation_risks": "同步对穿（delays 全 0）→ 中心碰撞门必炸；错峰不足（如 i*150 但飞行远长于错峰跨度）→ 仍在交叉点重叠→碰撞门。",
...
COMPOSITE_SKILLS = [
    ...
    },
    {
        ...
        "uses": ["call_response_safe", "safe_move"],
        "category": "grouping",
        "role_fit": ["buildup", "development"],
        "music_fit": "对答乐句、左右呼应、交替强拍。",
        "visual_effect": "左组先动、右组亮灯应答，再由右组接上；两组色彩呼应，读出问答结构，同时路线由本地 safe_assign 保底。",
        "constraints": "首选 `call_response_safe`。分组要可分（left_right/front_back）；geo 写稀疏的两带/弧形/route-around 点表，实际相邻约70-100cm，但不要手动写 `min_xy_cm=90` 增加自杀门槛。总时长=2*flying+gap 填满窗口；例如 S02 8.7s 写 flying_ms=4000, gap_ms=700。不要裸写 `group_relay(best_assign(...))`，那是旧坑。",
        "how_to_choose": "音乐有明显一问一答/对称乐句时。",
        "avoid_overuse": "连续多段问答会单调——配合其他母题交替。",
        "example": "geo = custom_points([...两带/弧形 route-around 问答点...], n=len(drones))\nprev = call_response_safe(drones, prev, geo, 4000, gap_ms=700, relay_split='left_right', colors=('#ff6040','#4060ff'), gradient_to=('#ffd166','#66e0ff'))",
    },
    {
        "name": "chain-follow-phrase",
        "uses": ["chain_follow_safe"],
        "category": "motion",
        ...
        "example": "ctrl = [(90,420,180), (260,330,150), (500,120,190)]\nprev = chain_follow_safe(drones, ctrl, hop_ms=700, lag_hops=1, spacing_cm=65, colors=palette)",
    },
    {
