...
def test_crossing_collision_directs_safe_relay_recipe():
    ...
    # (call_response_safe / safe_move relay) so the model stops re-deriving it across rounds.
    result = ValidationResult(
        compile_ok=True,
        run_ok=True,
        read_fii_ok=True,
    ...
    assert "call_response_safe" in fb and "safe_move" in fb, "must name the zero-crossing recipe"
...
def test_endpoint_proximity_does_not_trigger_crossing_diagnosis():
    ...
    # crossing — must NOT emit the safe relay crossing prescription (avoid over-firing).
    result = ValidationResult(
        compile_ok=True,
        run_ok=True,
        read_fii_ok=True,
    ...
    fb = result.repair_feedback()
    assert "call_response_safe" not in fb, "endpoint proximity must not trigger the crossing recipe"
