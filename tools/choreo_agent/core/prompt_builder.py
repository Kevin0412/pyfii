...
- 不要直接写 `inittime()`；跨段对齐由 `auto_init` 处理"""
    ...
    if segment_upper in {"S04", "S05"}:
        assign_rule = "- S04/S05 大动作配错峰/分组时**优先** `delays = ripple_delays(prev, ...)` 后 `targets = safe_assign(prev, geo, delays=delays, flying_ms=...)`（按真实分时轨迹选无碰撞排列，避免“算着安全实跑撞”反复返工）；不错峰的同步大交换才用 `far_assign(prev, geo, min_path_cm=active_min_path_cm(flying_ms))`；就近承接用 `best_assign(prev, geo)`。同步转场路径 <51cm 时：best 已是最优排列、改点表救不了——能换排列就改 far_assign，点本身太近（对穿/密集）就显著拉开点表、或 `safe_assign(prev, geo, delays=delays, flying_ms=...)` 按真实分时轨迹验真挑排列（对穿想保留就把错峰放大到顺序通过，别靠 mirror 小错峰硬穿——照撞返工）。每个 keyframe 最长路径有确定性上限（speed×时长内能飞完），规划报告会给精确 `[下限,上限]` 带——别设计飞不完的大动作。返回值都是 targets 列表，不要拆 `perm/min_d`"
    else:
        assign_rule = "- `targets = best_assign(prev, geo)` 就近承接；同步转场路径 <51cm 时 best 改点表无效（已是最优排列）：能换排列改 `far_assign(prev, geo, min_path_cm=active_min_path_cm(flying_ms))`，点本身太近（对穿/密集）就显著拉开点表、或 `safe_assign(prev, geo, delays=delays, flying_ms=...)` 按真实分时轨迹验真挑排列（mirror 对穿小错峰照撞，别用来硬穿）。prev/geo 用完整 `(x,y,z)`，返回值就是重排后的 targets 列表，不要拆 `perm/min_d`"
