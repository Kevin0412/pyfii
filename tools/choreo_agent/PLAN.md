...
## 0.6 导演指令服从率矩阵（2026-07-04 首轮基准，18 类真实 API 验收）

| # | 指令类 | flash | mimo | qwen-nothink | qwen-think* |
|---|---|---|---|---|---|
| 1 | 精确坐标+身份保持 | ✅ | ❌ | ❌ | ❌(安全过,偏243cm) |
| 2 | 全体单色灯 | ✅ | ❌ | ❌ | ✅ |
| 3 | 组合一次性 | ❌ | ❌ | ✅ | — |
| 4 | 模糊方位(中间偏左) | ✅ | ❌ | ❌ | ❌(差32cm) |
| 5 | 相对微调两段式 | ❌ | ❌ | ❌ | — |
...
| 17 | 整段限速 | ✅(带速度图例) | ❌ | ❌ | — |
| 18 | 否定约束 | ⚠️不稳定 | ❌ | ❌ | — |
| | **稳定通过** | **9/18** | **3/18** | **3/18** | **5/18**(全量) |

口径注：flash 预算较宽（attempts 4/修正 2）；mimo/qwen 用紧预算（attempts 3/修正 1/保险丝 8）
——mimo 的"能到但磨"风格在紧预算下吃亏明显。qwen-think 全量通过集 = {2 灯光, 9 预检,
11 同步爆闪, 13 定格, **17 限速**}——独占速度数学类，think 买到的是计算与时序纪律；
排名 flash 9 > qwen-think 5 > qwen-nothink 3 = mimo 3。case 6/14 四配置全零（终审定案）。

修正（2026-07-04 P3 共享舞台敏感性复测，flash case1/16 各 3 次）：**入口几何假说被证伪**——
中性舞台上 case 1 三连败（偏差 406cm/安全失败/未中），case 16 的 stage A 反而命中过一次
14.1cm。位置类指令在 flash 上是**纯随机波动（通过率 ~1/6–2/6）**，与舞台无关；
"身份保持"措辞是必要非充分条件。此前 case 1 的 0.0cm 与 case 16 的三连败均为方差两端。

关键结论：
- **三杠杆措辞**（身份保持写法/速度换算式/"每一架都要"）是 flash 从失败翻通过的直接原因
- **零通过终审**：case 12 由 qwen-nothink 通过洗清（断言没问题）；case 6/14 三模型全零
  ——6 待断言复审（"依次+明暗渐变"双重要求或过复合），14 = 档位上限+分配工具链摩擦，留作进阶标杆
- 组合指令的产品路径 = 逐条下达（case 15 两段式语义已验证；16 的 stage A 失败是位置类
  入口几何敏感，非逐条机制问题）
- 反常互补：qwen-nothink 独过 3/12，mimo 与 flash 通过集重叠——弱模型间能力剖面差异真实存在

## 0.7 矩阵裁决史（护栏协议执行记录）

- **C6 prompt 批次**（反馈注入等）：flash 18.5→18.5 持平干净；mimo 18→28 形式踩线，
  归因方差（三样本 23/39/22，黑洞段不重合、类别谱系同基线）→ C6 保留；协议修正"基线≥2 次"
- **C14 缓存前缀重构**：flash **18.5→14 轮、成本 -26%**（明确提升）；mimo 1/2 完成
  （B 跑 S06 黑洞 20 轮致死——**第三次不同段黑洞**：S03/S04/S06 各一次，先于 C14 存在）、
  A 跑 24 轮在典型带内 → 形式 REGRESSED，归因维持方差主导；C14 保留。
  待办：mimo 黑洞的结构性对策（如黑洞段自动降级重规划）列入后续计划；
  mimo 对 planning 阶段新增 system prompt 的兼容性保持观察（下轮矩阵若再劣化则做
  `_is_mimo_provider` 定向回退实验）

## 0.7b 第二阶段矩阵裁决（续 0.7）

- **P2 设计卡前传**（生产 prompt 字节变更）：flash 4 跑 3 完成——唯一失败是熔断生效前的
  40 轮离群（S02 window_fill 磨 + S04 黑洞），确认对 2/2 @ 14+14 轮与 C14 基线持平；
  mimo 对 **2/2 @ 25/25 轮，零黑洞**（战役以来最稳）。跨段语义实测 2/2（呼应 0.0cm 残差、
  蓄力 0.62 速比）→ **P2 保留**。熔断（P6）在确认对中在场，黑洞未再现。

## 0.7c P6 结论（2026-07-04）

- **case 6 定位完成**：拆分后两个单要求均可达——21 时序换色（修正 onset 测量为 RGB 匹配 +
  给精确数字反馈后：0 逆序、2.05s 跨度、完美左→右）、22 呼吸（7/7 幅度 209）。复合 6 的
  四配置全零 = **组合上限**（与 case 3 同根），产品对策同为逐条下达。灯光类指令统一豁免
  退化对比（模型靠定点站位完成灯效属合理调度；运动类指令保留对比）
- **黑洞熔断**上线并在 P2 确认对中在场（黑洞未再现）；离线测试驱动验证重置路径
- **qwen thinking pipeline 完整跑通**：S01–LAND 7/7、31 轮、1.8 小时（S06 磨 14 轮但完成）
  ——本地模型在保险丝+单候选下可用于全流程测试
...
**Phase 3+4+5+6 合并矩阵裁决**（三段式规划历史接入 + K 候选归并 + auto-compact +
REPL 可观测性，四个 phase 一起验，因为都是同一批同时开发/同时用一次真实全流程验证过的）：

| | 基线(重构前) | Phase 2a | Phase 2b | Phase 3-6 |
|---|---|---|---|---|
| flash 轮数 | 14 (2/2) | 11 (2/2) | **9.5 (2/2)** | 17 / 25 / **FAILED**（2/3） |
| mimo 轮数 | 25/25 (2/2) | 25 (1/2，token bug) | **15/15 (2/2)** | 12/18，均值15 (2/2) |

mimo：与 Phase 2b 基线**完全持平**（15→15），`NO REGRESSION`，核心改动（给模型自己的代码）
带来的 25→15（-40%）改善在 mimo 上稳定保留。

flash：3 个样本里 2 个正常（17 轮完成；25 轮——S01 撞见一个运行期 bug，`cos`/`sin` 其实
已在 `function.py` 里正常导出，真实原因未深挖到底但 scheme reset 一次就救回来了，最终完成），
第 3 个样本**没完成**——卡在 S06（`action_incomplete`，4 cycle，1 次 scheme reset 没能在
剩余预算内救回来）。S06 是否受 Phase 3 影响无法完全排除（S06 有前段 exit_state，三段式
规划真的会跑，不像 S01 那样天然不涉及 Phase 3 代码），但 S06/S03/S04 一类"偶尔黑洞"在
这个 agent 的完整历史里反复出现（预 C14、C14、Phase 2a 的 mimo、现在这次），跨越完全不
相关的代码改动窗口——用户判断："基本上在不降低难度的情况下没有稳定过的，问题不大"，
即这类偶发黑洞是系统一贯的已知特征，不是本次改动引入的新问题。

**结论：Phase 3-6 保留**。核心机制（2a+2b）的改善经完整矩阵证实且在 mimo 上于 3-6 阶段
持续保持；flash 在个别历史已知难点上的偶发失败被判定为系统固有变现，非本次回归。
...
# 13. 跨音乐差异化验收报告 (2026-06-12)

三首音乐零手工编辑全流程通过（S01→S07+LAND），三场演出结构互异——hardcode 验收通过：

| | 无人区"脉冲星觉醒" | 春节序曲(误读为"暗夜星尘") | 云宫迅音"天宫序曲：神话觉醒"(带曲名) |
|---|---|---|---|
| 段窗 | 14/24.8/34/42.7/52.2/60.7 | 12.9/18.8/27.1/36.6/50.1/62.8 | 9.1/19/29.9/39.6/51.2/63.8 |
| 语体倾向 | 灯光主导(42色/113tick) | 灯光主导(55色/97tick) | 队形主导且灯光丰富(59色/109tick) |
| 质心叙事 | 钉守 28cm | 出走-回归 28/0 | 钉守 16cm(宫殿) |
| readable_ratio | 0.25 | 0.27 | 0.64(超 dntg 0.61) |
| settled readable | 27% | 29% | 71% |
| LAND | 编舞降落 bug(已修) | 同 bug(已修) | 协议完美+真实落地 |

结论：
1. 语体随音乐切换（暗/静→灯光主导，史诗→队形主导）——无系统性灯光偏置。
